...
int usb_snd_control_msg(int dev_handle, int requesttype, int request, 
...
int usb_rcv_control_msg(int dev_handle, int requesttype, int request, 
                    int value, int index, char *bytes, int size, int timeout)
{
    HID_DEV_T       *hid_dev;
    USB_DEV_T       *udev;
...
#define BUFLEN          9
    ...
    len = usb_rcv_control_msg(hidif->dev_handle,
            USB_ENDPOINT_IN+1,
                                    ...
                                    uint32_t depth, char *buffer, uint32_t size)
{
    ...
    len = usb_snd_control_msg(hidif->dev_handle,
 ...
 * @retval   Otherwise   Failed
 */
int32_t USBH_HidSetReport(int32_t i32Hanlde, int ReportID, char * buffer, uint32_t size)
{
    HIDInterface * hidif = (HIDInterface *)i32Hanlde;
    int   len;
...
int32_t USBH_HidStartIntReadPipe(int32_t i32Hanlde, HID_INT_READ_FUNC *func)
{
    ...
    ep_info = hid_get_ep_info(hid_dev->udev, hid_dev->ifnum, USB_DIR_IN);
    if (ep_info == NULL)
    {
...
static void  hid_write_irq(URB_T *urb)
{
    ...
    if (!hid_is_opened(hidif)) 
    {
        HID_DBGMSG("hid_write_irq - the device have been closed, terminate it.\n");
        return;
    }
...
int32_t USBH_HidStartIntWritePipe(int32_t i32Hanlde, HID_INT_WRITE_FUNC *func)
{
    ...
    ep_info = hid_get_ep_info(hid_dev->udev, hid_dev->ifnum, USB_DIR_OUT);
    if (ep_info == NULL)
    {
