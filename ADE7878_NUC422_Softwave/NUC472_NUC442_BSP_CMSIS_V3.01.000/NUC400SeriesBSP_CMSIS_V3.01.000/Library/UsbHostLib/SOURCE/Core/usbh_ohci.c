...
static void TD_CompletionCode(uint8_t cc)
{
    ...
    {
       case TD_CC_NOERROR:
            USB_debug("NO ERROR"); break;
       case TD_CC_CRC:
            USB_debug("CRC ERROR"); break;
       case TD_CC_BITSTUFFING:
            USB_debug("BIT STUFFING"); break;
       case TD_CC_DATATOGGLEM:
            USB_debug("DATA TOGGLE MISMATCH"); break;
       case TD_CC_STALL:
            USB_debug("STALL"); break;
       case TD_DEVNOTRESP:
            USB_debug("DEVICE NOT RESPONDING"); break;
       case TD_PIDCHECKFAIL:
            USB_debug("PID CHECK FAILURE"); break;
       case TD_UNEXPECTEDPID:
            USB_debug("UNEXPECTED PID"); break;
       case TD_DATAOVERRUN:
            USB_debug("DATA OVERRUN"); break;
       case TD_DATAUNDERRUN:
            USB_debug("DATA UNDERRUN"); break;
       case TD_BUFFEROVERRUN:
            USB_debug("BUFFER OVERRUN"); break;
       case TD_BUFFERUNDERRUN:
            USB_debug("BUFFER UNDERRUN"); break;
       case TD_NOTACCESSED:
            USB_debug("NOT ACCESSED"); break;
    }
    USB_debug(",  [0x%02x]\n", cc);
}
...
static void urb_print(URB_T * urb, char * str, int small)
{
    ...
    if (!small) 
    {
        ...
        if (usb_pipecontrol (pipe)) 
        {
            ...
            {
                case 0:  USB_debug("GET_STATUS"); break;
                case 1:  USB_debug("CLEAR_FEATURE"); break;
                case 3:  USB_debug("SET_FEATURE"); break;
                case 5:  USB_debug("SET_ADDRESS"); break;
                case 6:  USB_debug("GET_DESCRIPTOR"); break;
                case 7:  USB_debug("SET_DESCRIPTOR"); break;
                case 8:  USB_debug("GET_CONFIGURATION"); break;
                case 9:  USB_debug("SET_CONFIGURATION"); break;
                case 10: USB_debug("GET_INTERFACE"); break;
                case 11: USB_debug("SET_INTERFACE"); break;
                case 12: USB_debug("SYNC_FRAME"); break;
                default: USB_debug("Command"); break;
            }
            ...
            else 
            {
                for ( i = 0; i < 32; i += inter) 
                {
                    ...
                    for (ed_p = &(ohci.hcca->int_table[ep_rev (5, i)]); 
                             *ed_p != 0; ed_p = &(((ED_T *) (*ed_p))->hwNextED)) 
                         inter = ep_rev (6, ((ED_T *) (*ed_p))->int_interval);
                    *ed_p = (uint32_t)ed; 
                }       
                ed->ed_prev = NULL;
...
static int  ep_unlink(ED_T *ed) 
{
    ...
    switch (ed->type) 
    {
        ...
        case PIPE_INTERRUPT:
            ...
            {
                int  count = 0;
                for (ed_ptr = (uint32_t *)(ohci.hcca->int_table[ep_rev (5, i) + int_branch]), inter = 1; 
                       (*ed_ptr != 0) && (*ed_ptr != ed->hwNextED); 
                       ed_ptr = (uint32_t *) ((ED_T *)ed_ptr)->hwNextED, 
                       inter = ep_rev (6, ((ED_T *)ed_ptr)->int_interval), count++) 
                {
                    if ((ED_T *)ed_ptr == ed) 
                    {
                        ed_ptr = (uint32_t *)ed->hwNextED;           
                        break;
                    }
                    if (count > 30)
                        break;
        ...
        case PIPE_ISOCHRONOUS:
            ...
            else 
            {
                for (i = 0; i < 32; i++) 
                {
                    for (ed_ptr = (uint32_t *)ohci.hcca->int_table[ep_rev (5, i)]; 
...
{    
    ...
    if (!ohci.disabled) 
    {
        ...
        USBH->HcIntSts = OHCI_INTR_SF;
        USBH->HcIntEn = OHCI_INTR_SF;
    }
}
...
{ 
    ...
    switch (usb_pipetype (urb->pipe)) 
    {
        case PIPE_BULK:
            ...
            cnt++;
            USBH->HcComSts = OHCI_BLF;      /* start bulk list */
            break;
        ...
        case PIPE_CONTROL:
            ...
            USBH->HcComSts = OHCI_CLF;          /* start Control list */
            break;
...
#define WR_RH_STAT(x)           (USBH->HcRhSts = (x))
#define WR_RH_PORTSTAT(x)       (USBH->HcRhPrt[wIndex-1] = (x))
#define RD_RH_STAT              (USBH->HcRhSts)
#define RD_RH_PORTSTAT          (USBH->HcRhPrt[wIndex-1])
...
static int  ohci_reset()
{
    ...
    USBH->HcIntDis = OHCI_INTR_MIE;

    /* Reset USB (needed by some controllers) */
    USBH->HcControl = 0;
        
    /* HC Reset requires max 10 ms delay */
    USBH->HcComSts = OHCI_HCR;
    
    while ((USBH->HcComSts & OHCI_HCR) != 0) 
    {
        if (--timeout == 0) 
        {
...
void USBH_IRQHandler(void)
{
    ...
    //printf("[irq-0x%x] 0x%x, 0x%x\n", ints, USBH->HcRhPrt[0], USBH->HcRhPrt[1]);
    ...
    if (ints & OHCI_INTR_SO) 
    {
        ...
        //while (1);
        //USB_error("Error! - USB Schedule overrun, count:%d\n", (USBH->HcComSts >> 16) & 0x3);
        USBH->HcIntDis = OHCI_INTR_SO;
        USBH->HcIntSts = OHCI_INTR_SO;
    }
    ...
    {
    	USBH->HcIntDis = OHCI_INTR_RHSC;
    	USBH->HcIntSts = OHCI_INTR_RHSC;
	}
	
	if (ints & OHCI_INTR_RD)
	{
    	USBH->HcIntDis = OHCI_INTR_RD;
    	USBH->HcIntSts = OHCI_INTR_RD;
	}

    ints &= ~OHCI_INTR_WDH;
    USBH->HcIntEn = OHCI_INTR_MIE;
...
static int  hc_start ()
{
    ...
    USBH->HcCtrHED = 0;             /* control ED list head */
    USBH->HcBlkHED = 0;             /* bulk ED list head */
    ...
    USBH->HcRhDeA = (USBH->HcRhDeA | RH_A_NPS) & ~RH_A_PSM;
    USBH->HcRhSts = RH_HS_LPSC;

    /* POTPGT delay is bits 24-31, in 2 ms units. */
    usbh_mdelay((USBH->HcRhDeA >> 23) & 0x1fe);
