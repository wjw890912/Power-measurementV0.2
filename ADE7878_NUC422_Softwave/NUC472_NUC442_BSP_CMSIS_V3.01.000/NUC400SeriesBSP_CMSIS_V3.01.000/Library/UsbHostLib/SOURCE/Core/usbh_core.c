...
/**
  ...
  * @param    new_driver: The USB device driver.
  * @retval   0:  Success
  */
...
/**
  * @brief    Submit an URB to USB core for data transfers.
  * @param    urb: The USB transfer request to be processed.
  * @retval   0  Success
...
/**
  ...
  * @param    urb: The USB to be canceled.
  * @retval   0  Success
  * @retval   Otherwise  failed
  */
...
static int get_ctrl_msg_signal()  { return _Event_UrbCompleted; }
static void clr_ctrl_msg_signal()  { _Event_UrbCompleted = 0; }
...
/**
 *  @brief  Builds a control urb, sends it off and waits for completion
 ...
 *  @param  timeout      time to wait for the message to complete before timing out (if 0 the wait is forever)
 *  @retval   0  Success
 *  @retval   Otherwise  Failed
 *
...
/**
  ...
  * @param    pipe  Pipe description of the endpoint.
  * @retval   0    Success
  * @retval   Otherwise   Failed
  */
...
/**
  ...
  * @param    dev   The USB devic.
  * @param    interface    Interface number.
  * @param    alternate    Interface alternate setting number.
  * @retval   0  Success
  * @retval   Otherwise  Failed
  */
...
int USBH_Open(void)
{
    ...
    //USBH->OpModEn |= 0x8;     /* overcurrent setting */
    USBH->OpModEn &= ~0x8;      /* overcurrent setting */
...
int32_t USBH_Close(void)
{
    ...
    USBH->HcRhSts = USBH_HcRhSts_LPS_Msk;
...
int32_t USBH_Suspend(void)
{
    ...
    if (USBH->HcRhPrt[0] & 0x1)
       USBH->HcRhPrt[0] = 0x4;
    
    if (USBH->HcRhPrt[1] & 0x1)
        USBH->HcRhPrt[1] = 0x4;

    /* enable Device Remote Wakeup */
    USBH->HcRhSts |= USBH_HcRhSts_DRWE_Msk;

    /* enable USBH RHSC interrupt for system wakeup */
    USBH->HcIntEn =  USBH_HcIntEn_RHSC_Msk | USBH_HcIntEn_RD_Msk;    
...
int32_t USBH_Resume(void)
{
    ...
    if (USBH->HcRhPrt[0] & 0x4)
        USBH->HcRhPrt[0] = 0x8;
    if (USBH->HcRhPrt[1] & 0x4)
        USBH->HcRhPrt[1] = 0x8;
