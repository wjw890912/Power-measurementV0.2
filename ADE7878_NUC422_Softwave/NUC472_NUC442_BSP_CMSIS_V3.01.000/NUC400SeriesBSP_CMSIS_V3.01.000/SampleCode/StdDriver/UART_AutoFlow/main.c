 ...
 *           This sample code demo UART CTS/RTS auto flow operation.
 *           The sample code needs two boards. One is Master and the other is slave.
 *           Master will send 1k bytes data to slave.
 *           Slave will check if received data is correct after getting 1k bytes data.
 *
 * @note
...
void UART_TEST_HANDLE()
{
    ...
    uint32_t u32IntSts= UART1->INTEN;
...
void AutoFlow_FunctionTest()
{
    ...
    if(u8Item=='0') {
        for(u32i=0; u32i<(RXBUFSIZE-1); u32i++) {
            ...
            while(UART1->MODEM & UART_MODEM_RTSSTS_Msk);
        }
        printf("\n Transmit Done\n");
    } else {
        g_i32pointer = 0;
