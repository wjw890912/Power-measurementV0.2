...
static int mp3GetFrameLength(mp3Header *pHdr)
{
    ...
    int pV2L3Rates[] = {   0,   8,  16,  24,  32,  64,  80,  56,  64, 128, 160, 112, 128, 256, 320,  -1 };
    ...
    } else if (pHdr->layer == 0x03) {
        bitrate = pL1Rates[pHdr->bitrate];
    } else if (pHdr->version == 0x03) {
...
static void mp3PrintHeader(mp3Header *pHdr)
{
    ...
    audioInfo.mp3Channel = (pHdr->channel == 0x00 ? 2:1);
