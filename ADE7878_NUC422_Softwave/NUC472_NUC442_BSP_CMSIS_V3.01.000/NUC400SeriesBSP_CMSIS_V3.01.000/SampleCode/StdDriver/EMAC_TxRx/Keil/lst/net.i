...
typedef struct ARP_PACKET {
    ...
    uint8_t   au8SenderHA[6];    
    uint8_t   su8SenderIP[4];    
    uint8_t   au8TargetHA[6];    
    uint8_t   au8TargetIP[4];    
} ARP_PACKET;
...
static uint16_t s_u16IpPacketId = 1000;   
static uint32_t s_u32PktRdy;                




__align(4) static uint8_t  s_au8DhcpRawBuffer[3020];   
...
static uint8_t s_au8DhcpOptions[] = { 0x63, 0x82, 0x53, 0x63, 0x35, 0x01, 1 };
...
static uint16_t chksum(uint16_t *cp, int cnt)
{
    ...
    while (cnt--) {
        i2 = *cp++;
        i2 = ((((i2)>>8)&0xFF)|(((i2)<<8)&0xFF00));
        i1 += i2;
        if (i1 < i2)
            i1++;
    }
    return ((((i1)>>8)&0xFF)|(((i1)<<8)&0xFF00));
}
...
static void arp_reply(uint8_t *target_ip, uint8_t *target_mac)
{
    ...
    arp->u16Type = ((((0x0806)>>8)&0xFF)|(((0x0806)<<8)&0xFF00));
    arp->u16HType = ((((0x0001)>>8)&0xFF)|(((0x0001)<<8)&0xFF00));
    arp->u16PType = ((((0x0800)>>8)&0xFF)|(((0x0800)<<8)&0xFF00));
    arp->u8HLen = 6;
    arp->u8PLen = 4;
    arp->u16Operation = ((((0x02)>>8)&0xFF)|(((0x02)<<8)&0xFF00));
                       ...
                       uint8_t *pu8TxData, uint32_t u32Len)
{
    ...
    udp->u16Type = ((((0x0800)>>8)&0xFF)|(((0x0800)<<8)&0xFF00));

     
    udp->u8VerHLen = 0x45;      
    udp->u8ToS = 0;             
    udp->u16TLen = ((((sizeof(UDP_PACKET) - 14 + u32Len)>>8)&0xFF)|(((sizeof(UDP_PACKET) - 14 + u32Len)<<8)&0xFF00));
    udp->u16ID = ((((s_u16IpPacketId)>>8)&0xFF)|(((s_u16IpPacketId)<<8)&0xFF00));
    s_u16IpPacketId++;
    udp->u16Frag = 0;
    udp->u8TTL = 64;
    udp->u8Prot = 17;
    ...
    udp->u16HdrChksum = ~chksum((uint16_t *)&udp->u8VerHLen, 10);   

     
    udp->u16SrcPort = ((((u16SrcPort)>>8)&0xFF)|(((u16SrcPort)<<8)&0xFF00));
    udp->u16DestPort = ((((u16DestPort)>>8)&0xFF)|(((u16DestPort)<<8)&0xFF00));
    udp->u16MLen = ((((8 + u32Len)>>8)&0xFF)|(((8 + u32Len)<<8)&0xFF00));
    udp->u16UDPChksum = 0;
...
int process_rx_packet(uint8_t *pu8Packet, uint32_t u32Len)
{
    ...
    if (pu8Packet[0] == 0xFF) {      
        ...
        if ((!((arp->au8TargetIP[3]^g_au8IpAddr[3])|(arp->au8TargetIP[2]^g_au8IpAddr[2])|(arp->au8TargetIP[1]^g_au8IpAddr[1])|(arp->au8TargetIP[0]^g_au8IpAddr[0]))) &&
                (arp->u16Type == ((((0x0806)>>8)&0xFF)|(((0x0806)<<8)&0xFF00))) && (arp->u16Operation == ((((0x01)>>8)&0xFF)|(((0x01)<<8)&0xFF00)))) {
        ...
        if ((ip->u8Prot == 6) && (!((ip->au8DestIP[3]^g_au8IpAddr[3])|(ip->au8DestIP[2]^g_au8IpAddr[2])|(ip->au8DestIP[1]^g_au8IpAddr[1])|(ip->au8DestIP[0]^g_au8IpAddr[0])))) {
            
            return 0;
        }

        if ((ip->u8Prot == 17) && (udp->u16SrcPort == ((((67)>>8)&0xFF)|(((67)<<8)&0xFF00)))) {
        ...
        if ((ip->u8Prot == 17) && (!((ip->au8DestIP[3]^g_au8IpAddr[3])|(ip->au8DestIP[2]^g_au8IpAddr[2])|(ip->au8DestIP[1]^g_au8IpAddr[1])|(ip->au8DestIP[0]^g_au8IpAddr[0])))) {
        ...
        if ((ip->u8Prot == 1) && (!((ip->au8DestIP[3]^g_au8IpAddr[3])|(ip->au8DestIP[2]^g_au8IpAddr[2])|(ip->au8DestIP[1]^g_au8IpAddr[1])|(ip->au8DestIP[0]^g_au8IpAddr[0]))) &&
                (pu8Packet[34] == 0x08)) {
            IP_PACKET    *tx_ip;
            ...
            tx_ip->u16Type = ((((0x0800)>>8)&0xFF)|(((0x0800)<<8)&0xFF00));
            tx_ip->u8VerHLen = 0x45;          
            tx_ip->u8ToS = 0;                 
            tx_ip->u16TLen = ((((60)>>8)&0xFF)|(((60)<<8)&0xFF00));
            tx_ip->u16ID = ((((s_u16IpPacketId)>>8)&0xFF)|(((s_u16IpPacketId)<<8)&0xFF00));
            tx_ip->u16Frag = 0;
            tx_ip->u8TTL = 64;
            tx_ip->u8Prot = 1;
            ...
            tx_ip->u16HdrChksum = ~chksum((uint16_t *)&tx_ip->u8VerHLen, 10);   
...
int dhcp_start(void)
{
    ...
    dhcpTx->op_code = 1;
    dhcpTx->hw_type = 1;         
    dhcpTx->hw_len = sizeof(g_au8MacAddr);      
    dhcpTx->hops = 5;
    dhcpTx->tx_id = tx_id++;
    dhcpTx->seconds = 0;         
...
wait_offer:
    ...
    for (retry = 0; (retry < 16) && (!out); retry++) {
        ...
        udp_send(g_au8MacAddr, g_au8IpAddr, 68,
                 serverMAC, serverIP, 67,
                 (uint8_t *)dhcpTx, 236 + opt_len + 1);
        ...
        while(delay--) {
            if(s_u32PktRdy) {
                ...
                s_u32PktRdy = 0;
                NVIC_EnableIRQ(EMAC_RX_IRQn);
                if ((udp->u16DestPort == ((((68)>>8)&0xFF)|(((68)<<8)&0xFF00))) &&
                        (dhcpRx->op_code == 2) &&
